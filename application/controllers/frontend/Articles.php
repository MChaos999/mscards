...
class Articles extends FrontEndController
{
    ...
    public function item() {
        ...
        $articles_more = $this->articles_model->get_articles_more($this->clang);
        $this->data['articles_more'] = $articles_more;
        ...
        foreach(language(true) as $lang){
            $array = $lang.'_urls';
            $uri = 'uri'.strtoupper($lang);
            $this->{$array}[] = $article->{$uri};
        }

        $this->loadOGImgData($article);
        $this->_init_seo_data($article);

        $this->data['inner_view'] = 'pages/articles/item';
        $this->data['page'] = $page;
