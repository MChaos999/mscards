...
class Benefits extends BackEndController {
    protected $add;
    ...
    public function __construct() {
        ...
        $this->index_title = lang('Benefits');
        $this->add = lang('Add');
    ...
    public function index() {
        ...
        $objects = $this->benefits_model->find();
    ...
    public function put() {
        ...
        try {
            ...
            if (!$this->benefits_model->put($post)) {
    ...
    public function update_order() {
        ...
        try {
            ...
            if (!$this->benefits_model->update_sorder($post)) {
                throw new Exception(lang('Error writing data to table') . $this->main_page);
            }
    ...
    public function item($id = 0) {
        ...
        $item = $this->benefits_model->find_first($id);
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
                ...
                if (!$this->benefits_model->update($post, $id)) {
                    throw new Exception(lang('Error writing data to table') . $this->main_page);
                }
