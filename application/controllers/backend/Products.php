...
class Products extends BackEndController
{
    ...
    public function item($id = 0)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            try {
                ...
                if (!empty($this->input->post('more_products', true))) {
                    $new_related_products = $this->input->post('more_products', true);
                    unset($_POST['more_products']);
                    $this->db->where('product_id', $id)->delete('products_more');
                    foreach ($new_related_products as $k => $y) {
                        $this->db->insert('products_more', ['product_id' => $id, 'related_id' => $y]);
                    }
                }
                ...
                if (!empty($post['options'])) {
                    foreach ($post['options'] as $key => $options) {
                        if (empty($options['isShown'])) $options['isShown'] = 0;
                        if (empty($options['inUser'])) $options['inUser'] = 0;
                        $data = array(
                            'price' => $options['price'],
                            'titleRO' => $options['titleRO'],
                            'titleEN' => $options['titleEN'],
                            'sorder' => $options['sorder'],
                ...
                init_load_files($this->main_page);
                foreach (language(true) as $lang) {
                    if (!empty($_FILES['pdf' . strtoupper($lang)]['name'])) {
                        $this->upload->do_upload('pdf' . strtoupper($lang));
                        $file_data = $this->upload->data();
                        $file = $file_data['file_name'];
                        if (verify_img_extension($file_data['file_ext'])) $post['pdf' . strtoupper($lang)] = $file;
                    }
                }
        ...
        $item->options = $this->db->where('product_id', $id)->order_by('sorder ASC, id DESC')->get('products_options')->result();

        $categories = $this->categories_model->find();
        $media = $this->product_images_model->get_media_for_product($id);
        $filter_groups = $this->filter_groups_model->find();
        $filters = $this->filters_model->get_filters_by_filter_group_id(get_language_for_list(true), $item->filter_group_id);
        $bulks = $this->bulks_model->find();
        ...
        $this->data['title'] = lang('Edit') . $item->{'title' . get_language_for_list(true)};
        ...
        $products = $this->db->order_by("title" . get_language_for_list(true) . " ASC")->get('products')->result();
        $this->data['products'] = $products;
        $this->load->vars($this->data);
        $this->load->view($this->main_layout);
    }
    ...
    public function update_attributes($category_id)
    {
        ...
        $filter_groups_ids = array_map(function ($item) {
            return $item->filter_group_id;
        }, $query);
    ...
    public function add_variable()
    {
        ...
        echo '<tr id="variable' . $id . '">
                  <td width="50"> Сортировка
                  <input type="text" required class="form-control" value="" name="options[' . $id . '][sorder]">
                  </td>
                  <td > Название RO
                   <input type="text" required class="form-control" value="" name="options[' . $id . '][titleRO]">
                  </td>
                  <td > Название EN
                   <input type="text" required class="form-control" value="" name="options[' . $id . '][titleEN]">
                  </td> 
                  <td width="180"> Цена
                    <input type="text" required class="form-control" value="" name="options[' . $id . '][price]">
                  </td> 
                  <td width="120"> Активен
                    <input type="checkbox" style="width: 25px;height: 25px;" class="form-control" value="1" name="options[' . $id . '][isShown]">
                  </td> 
                  <td width="120">
                    <a onclick="DeleteVariable(' . $id . ')" class="btn btn-xs default btn-editable red-stripe" style="margin-top: 15px;">
